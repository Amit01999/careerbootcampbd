...
import { Search, BookOpen, Loader2, ChevronRight, AlertCircle } from 'lucide-react';
...
const PreliWritten = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        setLoading(true);
        const response = await preliWrittenService.getAll();
      ...
      <div style={{ background: SURFACE, borderBottom: `1px solid ${BORDER}` }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
          ...
          <p style={{ color: MUTED }} className="text-sm mt-2 ml-[52px]">
            Browse study materials for preliminary and written bank exams.
          </p>
        </div>
      </div>
              ...
              <div className="space-y-1.5">
                {filteredItems.map((item, i) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-4 px-4 py-3.5 rounded-xl text-left group hover:border-[#C49B4B]/30 hover:bg-[#1E1D1B] transition-all duration-200"
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      <span
                        style={{ color: MUTED, minWidth: 24 }}
                      ...
                      >
                        <BookOpen style={{ color: GOLD }} className="w-3.5 h-3.5" />
                      </div>
                      <span
                        style={{ color: TEXT }}
